...
If one of the RPC providers you’re using experiences failure or downtime, `Nexus` will cycle the request to another one, effectively multiplying your availability and uptime. Check out [Failure Recovery](/features/failure-recovery) for more information.
...
`Nexus` allows you to create fine-grained scopes via middlewares. If you're already using a token-based authentication system, you can configure your services to sign `Nexus` tokens as you authenticate your users. Check out [Authorization](/features/authorization) and [Middlewares](/features/middlewares) for more information.
